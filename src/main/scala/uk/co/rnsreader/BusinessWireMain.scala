...
//  val cutoffDate = date.minusMinutes(Integer.parseInt(System.getenv("CUTOFF_MINUTES_OFFSET")))
//
...
//          s"<tr><th style='width: 50%;'>Company Name Parsing still to be done</th><th><a href='${announcement.link}'>${announcement.title}</a></th></tr>" +
//          s"<tr><td style='width: 50%;'>Could not get content due to error</td><td>${e.getMessage}</a></td></tr>" +
//          "</table>"
//      }
//
//      case \/-(matches) => {
